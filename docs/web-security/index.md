# Web Security

This section covers core web application security topics and common attack classes.

The goal is to organize practical notes around the vulnerabilities, weaknesses, and testing areas that matter most in real-world web security review work.

## Included topics

- [XSS](xss.md)
- [SQL Injection](sql-injection.md)
- [SSRF](ssrf.md)
- [LFI](lfi.md)
- [File Upload](file-upload.md)
- [Web Cache Poisoning](cache-poisoning.md)
- [Web Cache Deception](cache-deception.md)
- [HTTP Request Smuggling](request-smuggling.md)

This area is intended to serve as a structured starting point for reviewing and understanding web security issues in a practical, methodical way.
